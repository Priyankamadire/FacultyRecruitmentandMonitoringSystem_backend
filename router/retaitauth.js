...
router.post('/rfaculty', async (req, res) => {
    try {
        ...
        const detail = new Detail({ name,workedpost,instituteid,qualification,department,retairedyear,phone,email });
        await detail.save();
...
router.post('/rfaculty/:id',clgauthenticate, async (req, res) => {
    try {
        ...
        const facExists = await Retaire.findOne({ instituteid: instituteid, collegeId: collegeId });
        if (facExists) {
            return res.status(409).json({ error: "Faculty with the same institute ID already exists within the college" });
        }

        // Create a new faculty detail document
        const detail = new Retaire({ name,workedpost,instituteid,qualification,department,retairedyear,phone,email, collegeId: collegeId });
        await detail.save();
...
router.get('/rofaculty/:collegeId', async (req, res) => {
    try {
        ...
        const facultyDetails = await Retaire.find({ collegeId: collegeId });
